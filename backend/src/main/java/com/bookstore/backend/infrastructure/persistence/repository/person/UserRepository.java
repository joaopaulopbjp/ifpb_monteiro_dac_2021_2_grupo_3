...
public interface UserRepository extends JpaRepository<UserModel, Long> {
    ...
    @Query(value = "SELECT * FROM t_user JOIN (SELECT * FROM t_person JOIN (SELECT person_id FROM t_person_evaluate_join WHERE t_person_evaluate_join.evaluate_id = :evaluateId) AS retorno ON retorno.person_id = id) AS person ON person.person_id = t_user.id",nativeQuery = true)
    public Optional<UserModel> findByEvaluateId(@Param("evaluateId") Long evaluateId);
}
